...
async function loadTasks() {
    ...
    tasks.forEach(task => addTaskRow(task.id, task.todo, task.userId, task.completed));
...
search.addEventListener("input", function() {
    ...
    rows.forEach(row => {
        const todoText = row.cells[1].textContent.toLowerCase().trim();
...
addTaskForm.addEventListener("submit", function(event) {
    ...
    const task = { id: taskIdCounter++, todo: taskText, userId: 1, completed: false };
...
function addTaskRow(id, todo, userId, completed) {
    ...
    tr.innerHTML = `
        <td>${id}</td>
        <td ${completed ? 'class="line-through"' : ''}>${todo}</td>
        <td >${userId}</td>
        <td>${completed ? "Completed" : "Pending"}</td>
        <td>
            <section class="btn-group">
            
            <button class="btnn btn-done" onclick="openCompleteModal(this.closest('tr'))"> <span title="done">&#10003;</span> </button>
            <button class="btnn btn-del" onclick="openModal(this.closest('tr'))"> <span title="delete">&#10007;</span> </button>
            <button class="btnn btn-edite" onclick="editTask(this.closest('tr'))"> <span title="edite">&#9998;</span> </button>
            </section>
        </td>
    `;
